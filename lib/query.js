...
export async function getPostBySlug(slug) {
  const query = `{ category(idType: SLUG, id: "${slug}") { name posts { nodes { slug title(format: RENDERED) uri date excerpt(format: RENDERED) featuredImage { node { altText mediaDetails { sizes { sourceUrl width height } } } } } } } }`
...
export async function getPostByTag(slug) {
  const query = `{ tag(id: "${slug}", idType: SLUG) { posts { nodes { slug title(format: RENDERED) uri date excerpt(format: RENDERED) featuredImage { node { altText mediaDetails { sizes { sourceUrl width height } } } } } } } }`
...
export async function getPostByAuthor(slug) {
  const query = `{ user(id: "${slug}", idType: SLUG) { posts { nodes { slug title(format: RENDERED) uri date excerpt(format: RENDERED) featuredImage { node { altText mediaDetails { sizes { sourceUrl height width } } } } } } } }`
...
export async function getPostBySearch(search) {
  const query = `{ posts(where: {search: "${search}"}) { nodes { date featuredImage { node { altText mediaDetails { sizes { sourceUrl width height } } } } excerpt(format: RENDERED) title(format: RENDERED) uri slug } } }`
